...
$('#transform-js').on('click', function (evt) {
    var text =  "// ==UserScript==\n" +
                "// @name         Custom Tweakers Userscript\n" + 
                "// @namespace    http://koenv.com\n" + 
                "// @version      0.1\n" + 
                "// @description  Tweakers Styles\n" + 
    ...
    $.each(lines, function(i, line) {
        text = text + line.replace(/'/g, "\\'") + ' \\\n';
    });
    text = text + '\'; \n\n';
    text = text + 'addGlobalStyle(text);'
    $('#output').val(text);
});
